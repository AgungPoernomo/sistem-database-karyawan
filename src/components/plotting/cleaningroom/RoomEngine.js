// src/components/plotting/cleaningroom/RoomEngine.js
...
export function updateRoomStats(dataToRender) {
    let pria = 0, wanita = 0; 
    ...
    const statsContainer = document.getElementById('statsContainerR');
    if(statsContainer) {
        statsContainer.innerHTML = `
            <div class="space-y-3">
                ...
                <div class="flex justify-between items-center bg-cyan-50/80 dark:bg-cyan-900/30 border border-cyan-200 dark:border-cyan-500/40 px-4 py-3.5 rounded-2xl shadow-inner mt-2">
                    <span class="text-cyan-800 dark:text-cyan-300 font-black">TOTAL KAPASITAS NODE</span> <span class="text-cyan-600 dark:text-white font-black text-xl drop-shadow-[0_0_5px_rgba(14,165,233,0.5)]">${dataToRender.length}</span>
                </div>
            </div>
...
export function renderRoomTable(data) {
    const tableContainer = document.getElementById('tableContainerR');
    if (!tableContainer) return;

    if (data.length === 0) {
        tableContainer.innerHTML = `<div class="flex h-full items-center justify-center"><p class="text-slate-400 font-mono tracking-[0.2em] text-[10px] uppercase font-bold bg-white/50 dark:bg-slate-900/50 px-6 py-3 rounded-xl border border-white dark:border-slate-700/50">Kosong. Tidak ada PIC di area ini.</p></div>`; 
        return;
    }
