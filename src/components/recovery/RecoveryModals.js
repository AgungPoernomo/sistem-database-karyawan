...
export function initRecoveryModals(containerId, onAcceptPolicy) {
    ...
    container.innerHTML = `
        <div id="policyModal" class="fixed inset-0 z-[9999] flex flex-col items-center justify-center opacity-0 transition-opacity duration-500 hidden">
            <div class="absolute inset-0 bg-slate-900/80 dark:bg-slate-950/95 backdrop-blur-xl"></div>
            <div class="relative z-10 w-full max-w-xl p-10 spatial-island rounded-[3rem] text-center transform scale-95 transition-all duration-500 border-t-amber-400 dark:border-t-amber-500/40 shadow-[0_20px_80px_rgba(245,158,11,0.2)] dark:shadow-[0_20px_80px_rgba(245,158,11,0.1)]" id="policyContent">
                ...
                <p class="text-sm text-slate-600 dark:text-amber-100/70 mb-10 leading-relaxed font-mono font-bold tracking-wide">
                    Anda memasuki Zona Pemulihan Data. Sesuai dengan protokol manajemen database, semua data di halaman ini akan <span class="text-rose-600 dark:text-rose-400 font-black uppercase">dihancurkan permanen</span> dari sistem secara otomatis jika telah melewati masa retensi <span class="text-rose-600 dark:text-rose-400 font-black border-b border-rose-400">30 Hari</span> sejak tanggal penghapusan.
                </p>
