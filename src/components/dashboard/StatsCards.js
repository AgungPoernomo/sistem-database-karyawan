...
function createClickableStatsWidget(title, contentHTML, onClickFunction) {
    return `
    <div class="spatial-island rounded-[2rem] p-6 relative overflow-hidden flex flex-col h-full min-h-[220px] cursor-pointer hover:shadow-[0_20px_40px_rgba(14,165,233,0.12)] hover:-translate-y-1.5 transition-all duration-300 group border-t border-t-white/80 dark:border-t-cyan-400/30 z-10" onclick="${onClickFunction}">
...
export function renderStatsCards(containerId, data) {
    ...
    // UBAH GRID MENJADI 3 KOLOM AGAR RAPI (Karena 1 widget dihapus)
    container.className = "grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-fr"; 
    ...
    html += createClickableStatsWidget('SEBARAN DATA', areaHTML, "window.dashboardApp.openAreaModal()");
